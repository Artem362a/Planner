...
from task_progress import sync_task_progress, sync_task_reminder
from schemas import *
...
def _sync_computed_day_reminders(db: Session, user: Any, d: date) -> None:
    """Re-anchor duration-mode reminders to the current day timeline."""
    cursor = _clock_to_minutes(_day_start_for_limit(db, user, d))
    rows = (
        db.query(DayTask)
        .filter(DayTask.user_id == user.id, DayTask.day == d)
        .order_by(DayTask.order_index.asc(), DayTask.id.asc())
    ...
    for raw in rows:
        ...
        elif getattr(task, "remind_lead_min", None) is not None:
            anchor_minutes = cursor % 1440
            task.remind_anchor_time = _time(
                anchor_minutes // 60, anchor_minutes % 60
            )
            task.remind_anchor_day_offset = cursor // 1440

        sync_task_reminder(db, user.id, task)
...
def create_task(
    ...
    db.flush()
    _validate_day_timeline_limit(db, current_user_row, d)
    sync_task_reminder(db, current_user_row.id, cast(Any, task))
    db.commit()
    db.refresh(task)
...
def update_task(
    ...
    sync_task_progress(
        db, current_user_row.id, task, old_status, subtasks_changed=body.subtasks is not None
    )
...
def reorder_day_tasks(
    ...
    db.flush()
    _validate_day_timeline_limit(db, current_user_row, d)
    _sync_computed_day_reminders(db, current_user_row, d)
    db.commit()
    return {"ok": True}
...
def carry_over_unfinished(
    ...
    for task in tasks:
        ...
        db.add(new_task)
        db.flush()
        sync_task_reminder(db, current_user_row.id, cast(Any, new_task))
        db.delete(task)
        moved += 1
